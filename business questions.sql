...
-- How many products of these tech categories have been sold (within the time window of the database snapshot)?  -- 16852
SELECT
...
-- What’s the average price of the products being sold? -- 108,50€ for tech products, 120,65€ in total
...
-- Are expensive tech products popular? -- 4148 products, 24,6% of total tech sales lie over the average price of 121€
...
-- How many months of data are included in magist database? -- 24 months
SELECT 
    TIMESTAMPDIFF(MONTH,
        MIN(earliest_date),
...
-- How many Tech sellers are there? -- 477
...
-- What percentage of overall sellers are Tech sellers? -- 15,4%
SELECT 
    (COUNT(DISTINCT CASE
            WHEN
...
-- What is the total amount earned by all sellers? 2.321.963,9€ of 16.008.872€
...
-- What is the total amount earned by all Tech sellers? 2.321.963,9€
...
-- Can you work out the average monthly income of all sellers? 172,7€ 
...
-- Can you work out the average monthly income of Tech sellers? 188€
...
--     What’s the average time between the order being placed and the product being delivered? -- 12,2 days
...
-- Whats the average delivery time for tech products? -- 12,7 days
SELECT 
    AVG(TIME_TO_SEC(TIMEDIFF(order_delivered_customer_date,
                    order_purchase_timestamp)) / 86400) AS avg_delivery_days
FROM 
...
-- 6535 deliveries delayed by at least 24hrs, while 2754 were delivered on the estimated day and 90.152 were delivered earlier.
...
-- Delayed deliveries of packages under 10kg
SELECT 
    orders.order_id,
    product_weight_g,
...
-- Sale trend of profits for tech products
SELECT 
    order_purchase_timestamp,
    payment_value,
